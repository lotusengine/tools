...
import { CollectionModel, CollectionMapping, CollectionCreateInput, CollectionUpdateInput, CollectionOptions, CollectionQueries, CollectionTriggers } from 'typings/collection'
...
export const findCollection = async (id: UUID): Promise<CollectionModel> => {
  ...
  const { service, options, mapping, queries, triggers, label, createdAt, updatedAt } = parseFields<{
    service: {
      id: UUID
    }, options: CollectionOptions, queries: CollectionQueries, triggers: CollectionTriggers, mapping: CollectionMapping, label: string, createdAt: ISO8601DateTime, updatedAt: ISO8601DateTime
  }>(res.collection, ['options', 'mapping', 'queries', 'triggers'])

  return { id, createdAt, updatedAt, serviceId: service.id, label, options, mapping, queries, triggers }
}
...
// List collections
export const listCollections = async (): Promise<Partial<CollectionModel>[]> => {
  ...
  const res = await doQuery<{ collections: { nodes: CollectionModel[] } }>(query)

  return res.collections.nodes.map(collection => parseFields<{ id: UUID, label: string, createdAt: ISO8601DateTime }>(collection, ['options', 'mapping', 'queries', 'triggers']))
}
