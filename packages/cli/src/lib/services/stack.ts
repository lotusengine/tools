...
import { ensureDir, readdir, readFile, readJson } from 'fs-extra'
import { camelCase, paramCase } from 'lib/utils/formaUtils'
...
import { doMutation, removeEmpty } from 'lib/query'
...
export const ensureEmptyDir = async (dir: string): Promise<void> | never => {
  await ensureDir(dir)
...
export const generateStack = async (
  ...
  const { app } = await loadConfig(projectPath)
...
// Deploy a stack
export const deployStack = async (params: StackDeployInput) => {
  const query = `mutation StackDeploy($input: StackDeployInput!) {
    stackDeploy(input: $input) {
        id
      }
    }`
