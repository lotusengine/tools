...
import { ViewModel, ViewCreateInput, ViewUpdateInput } from 'typings/view'
...
export const findView = async (id: UUID): Promise<ViewModel> => {
  ...
  const { service, label, createdAt, updatedAt } = parseFields<{
    service: {
      id: UUID
    }, label: string, createdAt: ISO8601DateTime, updatedAt: ISO8601DateTime
  }>(res.view, [])
...
export const listViews = async (): Promise<Partial<ViewModel>[]> => {
  ...
  return res.views.nodes.map(view => parseFields<{ id: UUID, label: string, createdAt: ISO8601DateTime }>(view, []))
}
